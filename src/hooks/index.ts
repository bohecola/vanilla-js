...
// 返回所有 Demo 的路径（用于侧边栏「Demo 片段」分组）
 ...
 *
 * 路径去掉 glob 前缀，落到磁盘上就是同一套目录结构 —— 也就是说磁盘上的内容和
 * 侧边栏里列出来的严格一致：`data/index.js`（glob 里排除掉的）和 `other/bst.html`
 * （glob 只要 .js）界面上没有，磁盘上也不该凭空多出来。
 *
